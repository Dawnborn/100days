# -*- coding: utf-8 -*-
"""
Created on Fri Mar 27 15:51:49 2020
...
def smoSimple(dataMatin, classLabels, C, toler, maxIter):
    ...
    while (iter < maxIter):
        ...
        # 记录安排alpha还是否已经进行优化， 每次循环时设为0， 然后对整个集合顺序遍历
        alphaPairsChanged = 0
        for i in range(m):
            # print ('alphas=', alphas)
            ...
            if ((labelMat[i]*Ei<-toler) and (alphas[i] < C)) or ((labelMat[i]*Ei > toler) and (alphas[i] > 0)):
                ...
                # L和H用于将alphas[i]调整到0到C之间，若L==H，则不做改变直接进行continue
                # 若labelMat[i]不等于labelMat[j]，则说明异侧，二者相减，否则相加
                ...
                # eta是alpha[j]的最优修改量， 若为0则退for循环的迭代
                # 参考统计学习方法P125-128
                ...
                # 检测alphas[j]是否是轻微改变，若是则跳过
                if (abs(alphas[j] - alphaJold) < 0.00001):
                   print('j not moving enough')
                   continue
                
                # 然后alphas[i]进行同样幅度的移动，但方向相反
                alphas[i] += labelMat[i]*labelMat[j]*(alphaJold - alphas[j])

                # 在对alpha[i], alpha[j] 进行优化之后，给这两个alpha值设置一个常数b。
                # w= Σ[1~n] ai*yi*xi => b = yj- Σ[1~n] ai*yi(xi*xj)
                # 所以：  b1 - b = (y1-y) - Σ[1~n] yi*(a1-a)*(xi*x1)
                # a[1~n]中只有两项发生了变化，即编号为i和j的两项，故b1b2分别为
        ...
        # end for, 检查alpha是否更新，若是则iter置零继续运行程序
        # 直到更新完成后iter循环依旧无变化，退出循环
...
def smoSimple0(dataMatIn, classLabels, C, toler, maxIter):
    """smoSimple
